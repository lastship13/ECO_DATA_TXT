...
class MyMain(QMainWindow):
    def __init__(self):
        ...
        self.main_widget = MyApp(self)
        self.setCentralWidget(self.main_widget)

        # Tab Widget
        tabs=QTabWidget()
        tabs.addTab(MyApp(self), 'Title1')

########## 왜 FirstTab(self) 가 아니라 FirstTab() 이지??
        tabs.addTab(tab2.FirstTab(self), 'Title2')
########## 왜 FirstTab(self) 가 아니라 FirstTab() 이지??

        # QMainWindow 추가 --> 지금 탭 위치가 가운데 정렬 같은 느낌인데 이걸 바꿔서 위치 조정 가능할 듯
        ...
        saveAction.triggered.connect(partial(self.main_widget.file_btn_click_event, clicked_btn='save'))

        printAction = QAction(QIcon('print.png'), 'Print', self)
        printAction.setShortcut('Ctrl+P')
        printAction.setStatusTip('Print Search Results')
        printAction.triggered.connect(partial(self.main_widget.file_btn_click_event, clicked_btn='print'))
...
#        super().__init__()
    ...
    def initUI(self):
        ...
        self.RT_grid = self._create_RT_grid()

        self.action_grid = self._create_action_grid()
        self.arrow_grid = self._create_arrow_grid()  # Added by PH for multiple mass spectrum
        left_grid.addWidget(self.search_grid)
        left_grid.addWidget(self.info_grid)
        left_grid.addWidget(self.RT_grid)
        left_grid.addWidget(self.arrow_grid)
        left_grid.addWidget(self.action_grid)  # Added by PH for multiple mass spectrum
...
#        QTableWidget.setFixedHeight(self.info_table, groupbox.height()*4/10)
    ...
    # 2.2.1 Create Retention time grid
    def _create_RT_grid(self):
        groupbox = QGroupBox('Retention Time (RT) Data')

        action_layout = QHBoxLayout()

        predict_btn = QPushButton('Predict')
        predict_btn.clicked.connect(partial(self.file_btn_click_event, clicked_btn='predict'))  # Refer to 2.4.1 function

        self.rt_index = QLineEdit(self)
        self.rt_index.setAlignment(Qt.AlignCenter)
        self.rt_index.setText("-")
        self.rt_index.setReadOnly(True)

        action_layout.addWidget(self.rt_index)
    ...
    def _create_action_grid(self):
     ...
     # 2.3.2 Create action grid for multiple mass spectrum button by PH
    def _create_arrow_grid(self):
        groupbox = QGroupBox('Another Mass Spectrum Data')

        action_layout = QHBoxLayout()

        # Prev, next buttons
        prev_btn = QPushButton('Prev')
        prev_btn.clicked.connect(partial(self.arrow_btn_click_event, clicked_btn='prev'))  # Refer to 2.4.*  function
        next_btn = QPushButton('Next')
        next_btn.clicked.connect(partial(self.arrow_btn_click_event, clicked_btn='next'))  # Refer to 2.4.*  function

        self.le_index = QLineEdit(self)
        self.le_index.setAlignment(Qt.AlignCenter)
        self.le_index.setText("-/-")
        self.le_index.setReadOnly(True)

        action_layout.addWidget(self.le_index)
...
#        self.mass_fig = plt.Figure(figsize=(50,50))
        ...
        self.mass_fig.set_size_inches(12.5,7.0)
...
########################################################################################################################
    ...
    def search_btn_click_event(self):
        ...
        rt_cid, rt_data,rt_natcom = utils.load_RT()
        rt_cid = np.array(rt_cid, dtype=np.int)

        natcom_cid = rt_natcom['pid'].values
        natcom_rt = rt_natcom['scan'].values

        self.search = searched_cid

        self.rt_memory = "-"
        self.rt_w_natcom = "-"


        if searched_cid in rt_cid:
            cid_where = np.where(searched_cid == rt_cid)[0][0]
            print(type(rt_data[cid_where][0])) 
            self.rt_memory = rt_data[cid_where][0]
            self.rt_memory = round(self.rt_memory,2)
            if self.rt_memory <= 120:
                self.rt_memory = "≤ 120"


        if searched_cid in natcom_cid:
            cid_where = np.where(searched_cid == natcom_cid)[0][0]
            rt_where = natcom_rt[cid_where]
            rt_where = round(rt_where,2)
            self.rt_w_natcom = natcom_rt[cid_where]
            self.rt_w_natcom = round(self.rt_w_natcom,2)

        else:
            rt_where="None"

        if rt_where == "None":
            self.rt_index.setText(str(rt_where))
        else:
            self.rt_index.setText("{}  sec".format(str(rt_where)))
        self.rt_index.setReadOnly(True)
...
#       self.info_table.horizontalScrollBar().setValue(0)
        ...
        try:
            ...
            # (Added) Set mass spectrum index
            self.MS_index = 0
            #self.MS_max_index = len(self.MS_data[smiles]) - 1
            self.MS_max_index = len(self.MS_data[smiles])
            self.le_index.setText("1/"+str(self.MS_max_index))
...
#           ax = utils.plot_mass(ax, self.mz_array, self.intensity_array)
            ...
            idx_ms = 0
            self.msinfo_table.setRowCount(len(self.MS_data[smiles][0]['metaData']))
            meta_n=[]
            meta_v=[]
...
#           self.mass_layout.addWidget(self.save_print_group)
    ...
    # 2.4.* Multi mass spectrum by PH
    def arrow_btn_click_event(self, clicked_btn):
        if clicked_btn == 'prev':  self.MS_index -= 1
        elif clicked_btn == 'next': self.MS_index += 1

        if self.MS_index == -1 : self.MS_index = 0
        elif self.MS_index == self.MS_max_index  : self.MS_index = self.MS_max_index - 1
       ...
       #self.msinfo_table.setVerticalHeaderLabels(["              ", " ", " ", " "])
        ...
        self.le_index.setText(str(self.MS_index + 1)+"/"+str(self.MS_max_index))
        ...
        self.MS_data = utils.load_ms_info(smiles, self.msms_db)
        print(self.MS_index)
        searched_msms_data = self.MS_data[smiles][self.MS_index]['spectrum']
        self.mz_array = [float(d.split(':')[0]) for d in searched_msms_data.split(' ')]
        self.intensity_array = [float(d.split(':')[1]) for d in searched_msms_data.split(' ')]
        ...
        reference = self.MS_data[smiles][self.MS_index]['library']['link']
        self.reference_line.setText(reference)

        # (Added) Set Item and Label of msinfo_table
        idx_ms = 0
        self.msinfo_table.setRowCount(len(self.MS_data[smiles][self.MS_index]['metaData']))
        meta_n=[]
        meta_v=[]
        for ms_dict in self.MS_data[smiles][self.MS_index]['metaData']:
            self.header_list.append(ms_dict['name'])
            meta_n.append(ms_dict['name'])
            self.msinfo_table.setItem(idx_ms,0,QTableWidgetItem(str(ms_dict['value'])))
            meta_v.append(ms_dict['value'])
...
#           img_re.save('mass_fig2.png')
        ...
        elif clicked_btn == 'predict':
             #pred_check = QMessageBox.information(self, "Information","Do you want to predict Retention Time with HPLC-MS? \n\n(If predict value is less than 120, the predict may be not appropriate.)",QMessageBox.Yes | QMessageBox.Cancel, QMessageBox.Yes)
             pred_check = QMessageBox.information(self, "Information","Do you want to predict Retention Time with HPLC-MS? \n\n( non-retained molecules(RT ≤ 120 sec) are not predictable )",QMessageBox.Yes | QMessageBox.Cancel, QMessageBox.Yes)
             if pred_check == QMessageBox.Yes:
                 try:
                     if len(str(self.rt_w_natcom)) > 2:
                         #QMessageBox.about(self,"Notice","It is already Exp. data\n\npredict data is  {}sec" .format(self.rt_memory))
                         QMessageBox.about(self,"Notice","It is already Exp. data\n\npredict data is  %s sec" %self.rt_memory)
                         print("type(self.rt_memory)")
                         print(type(self.rt_memory))
                         self.rt_index.setText("{}  sec".format(str(self.rt_w_natcom)))
                         self.rt_index.setReadOnly(True)

                     else:
                         if len(str(self.rt_memory)) <= 2:
                             self.rt_index.setText("None")
                             self.rt_index.setReadOnly(True)
                             QMessageBox.about(self,"Notice","No predict data")
                         else:
                             self.rt_index.setText("{}  sec".format(str(self.rt_memory)))
                             #self.rt_index.setText(str(self.rt_memory))
                             self.rt_index.setReadOnly(True)
