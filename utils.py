...
def load_RT():
    with open('./RT.pkl','rb') as file:
        RT_data=pickle.load(file)

    with open('./natcom.pkl','rb') as file:
        RT_natcom=pickle.load(file)

    rt_cid_file = os.path.join(db_folder,'cid.txt')
    with open(rt_cid_file,'r') as file:
        RT_c = file.readlines()


    RT_cid=[]
    for line in RT_c:
        RT_cid.append(line.rstrip())

    return RT_cid, RT_data, RT_natcom
